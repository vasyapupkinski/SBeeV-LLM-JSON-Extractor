...
## Goal
Gemma-4-E4B를 DoRA 파인튜닝하여 채용 공고 → 구조화 JSON을 95%+ 유효율로 추출하는 특화 엔진 구축.
...
| 소스 | 수집량 | 특성 |
|------|:------:|------|
| 원티드 (wanted.co.kr) | ~400건 | 개발자 채용 주력, 상세 공고 |
| 직행 (zighang.com/it) | ~400건 | 28개 플랫폼 통합 애그리게이터 |

원본 800건 → 증강(지시어 변형 + 부분 추출 + 엣지 케이스) → 학습 데이터 2,000건+
