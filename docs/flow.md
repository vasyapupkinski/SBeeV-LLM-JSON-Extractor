...
flowchart TD
    subgraph "Phase 1: Data Collection"
        A[원티드 + 직행] -->|Crawl4AI| B["data/raw/ (800 .md files)"]
        B -->|품질 필터링| C["유효 원문 750건+"]
    end
    ...
    subgraph "Phase 3: Augmentation"
        D -->|전체 추출 750건| F[증강 데이터]
        D -->|부분 추출 750건| F
        D -->|지시어 변형 375건| F
        G[엣지 케이스 ~100건] --> F
        F --> H["data/splits/train.jsonl (1,800건)"]
        F --> I["data/splits/val.jsonl (200건)"]
    end
    ...
    subgraph "Phase 4: Training"
        ...
        J --> L["models/sbv-lora-adapter/ (비교용)"]
    end

    subgraph "Phase 5: Export & Deploy"
        K -->|save_pretrained_gguf| M["sbv-llm-q4_k_m.gguf"]
        M -->|ollama create| N[Ollama sbv-llm]
        K -->|push_to_hub| O[HuggingFace]
    end
    ...
    subgraph "Phase 6: Evaluation"
        ...
        L --> P
        P --> R["results/benchmark_report.md"]
    end
```
...
| 유형 | 변환 | 입력 → 출력 | 건수 |
|------|------|------------|:----:|
| 전체 추출 | 원본 그대로 | 원문 → 전체 JSON | 750 |
| 부분 추출 | 지시어 변경 | 원문 → `{tech_stack, experience}` only | 750 |
| 지시어 변형 | 프롬프트 3종 | 동일 원문, 다른 지시어 → 동일 JSON | 375 |
| 엣지 케이스 | null 다수 | 비개발/불완전 공고 → null 필드 | ~100 |
